...
| `DELETE r` (relationship variable) | Needs a parallel-edge probe (annotation-SID count for the matched edge) plus base-edge retraction by the bound edge — a new lowering. Will reuse the same conditional-write machinery (`WritePlan::Conditional`) as MERGE `ON MATCH SET` and bare `DELETE n`. |
