...
| Write — single-node `MERGE` | `MERGE (n:Label {props}) [ON CREATE SET …]` — find-or-create as a **single Txn**: the identifying pattern becomes a `NOT EXISTS` guard, so a fresh node + `ON CREATE SET` inserts fire only when no match exists; a match makes it a no-op | Depends on the SPARQL-UPDATE staging fix (zero-row WHERE = no-op). `ON MATCH SET`, relationship MERGE, and `MATCH … MERGE` are deferred. |
...
| MERGE `ON MATCH SET` / relationship MERGE / `MATCH … MERGE` | Single-node MERGE + `ON CREATE SET` now ship as a single Txn (see Done). `ON MATCH SET` fires on the complementary EXISTS branch, which one Txn can't co-express with the NOT EXISTS create branch — it needs a second guarded operation (API-level staging). Relationship MERGE and `MATCH … MERGE` are their own follow-ups. |
