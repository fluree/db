...
| Batched **edge** insert — `UNWIND $pairs AS p MATCH (a {id:p.from}),(b {id:p.to}) CREATE (a)-[:T]->(b)` | The edge-loading idiom (the scale bottleneck for anything past one request). Needs the UNWIND-of-maps desugared to an `UnresolvedPattern::Values` joined against id lookups — separate from the node unroll. Next increment. |
| Whole-map node params (`(n $props)`); `$map.field` outside UNWIND; nested/map field values | Standalone map-valued params (not via `UNWIND … CREATE`) still need a map-valued `Expr` the AST doesn't carry; deferred. |
