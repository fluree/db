...
| `MATCH (a)-[:A\|B*]->(b)` (unbounded var-length **alternation**) | ✅ | A type alternation in an *unbounded* transitive path lowers to a multi-predicate `PropertyPathPattern`; the closure follows an edge of any listed type per hop (LDBC IC12's `[:HAS_TYPE\|IS_SUBCLASS_OF*0..]`). Shares the engine with SPARQL `(a\|b)*`/`(a\|b)+`. Bounded alternation (`[:A\|B*1..3]`) still deferred (would need a per-hop union inside each fixed chain). |
