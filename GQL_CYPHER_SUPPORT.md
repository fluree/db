...
| Write — `MERGE … ON MATCH SET` | Resolved as a **conditional write** (`WritePlan`): the executor probes the writer view for the identifying pattern, then stages either the create branch (absent) or `MATCH (pattern) SET <on_match>` (present) | Probe + resolve happen inside the normal staging path (owned-ledger and server cached-handle alike), like UPSERT. The MERGE node must have a variable. Relationship MERGE and `MATCH … MERGE` are still deferred. |
...
| bare `DELETE n` / `DELETE r` | Both need a pre-write probe (relationship-existence for `DELETE n`; parallel-edge SID count for `DELETE r`). They will reuse the same conditional-write machinery (`WritePlan::Conditional`) that `MERGE … ON MATCH SET` now uses — a probe of the writer view that resolves to a concrete `Txn` or a typed error inside the normal staging path. |
