...
(defprotocol iNameService
  ...
  (-lookup [conn params] "Performs lookup operation on ledger address and returns latest commit address")
  (-alias [conn params] "Given a ledger address, returns ledger's default alias name else nil, if not avail")
  (-address [conn params] "Returns address/iri for provided ledger alias specific to the connection type")
  (-exists? [conn params] "Returns true if ledger exists (must have had at least one commit), false otherwise"))
