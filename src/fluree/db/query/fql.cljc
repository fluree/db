...
(defn query
  ...
  ([ds tracker query-map]
   (if (cache? query-map)
     ...
     (go-try
       (let [parsed-query    (parse/parse-query query-map)
             planned-query   (<? (optimize/-plan ds parsed-query))
             optimized-query (<? (optimize/-reorder ds planned-query))]
         (<? (exec/query ds tracker optimized-query)))))))
