...
(defn write-node
  ...
  [{:keys [index-store alias] :as _db} idx node updated-ids changes-ch error-ch]
  (go
      ...
      (try*
        (if (index/leaf? node)
          (do (log/debug "Writing index leaf:" display-node)
              (let [write-response (<? (storage/write-leaf index-store alias idx node))]
          ...
          (do (log/debug "Writing index branch:" display-node)
              (let [node*          (update-child-ids updated-ids node)
                    write-response (<? (storage/write-branch index-store alias idx node*))]
...
(defn refresh-index
  [{:keys [index-store] :as db} changes-ch error-ch {::keys [idx t novelty root]}]
  (let [refresh-xf (comp (map preserve-id)
                         (integrate-novelty t novelty))
        novel?     (fn [node]
                     (seq (index/novelty-subrange node t novelty)))]
    (->> (index/tree-chan index-store root novel? 1 refresh-xf error-ch)
            ...
            (let [error-ch   (async/chan)
              ...
              (async/alt!
                ...
                ([{:keys [garbage], refreshed-db :db, :as _status}]
                 (let [{:keys [index-store alias branch] :as refreshed-db*}
                       ...
                       garbage-res   (when (seq garbage)
                                       (let [write-res (<? (storage/write-garbage index-store alias branch t garbage))]
                                         (<! (notify-new-index-file write-res :garbage t changes-ch))
                                         write-res))
                       db-root-res   (<? (storage/write-db-root index-store refreshed-db* (:address garbage-res)))
