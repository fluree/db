...
`choose_chain_lane` (`default_graph_source.rs`) costs the three entry points in rows walked and picks the cheapest: the arena probe scales with the base edge (it buffers the base scan, capped at 20M rows), the enumeration with the whole arena (`f:reifiesSubject`'s merged count = live pairs, whatever the predicate), and the per-reifier chain with the reifier count times ~16 (one scattered point probe per sequential row — the crossover measured on the slice). The reifier count comes from the `f:reifiesSubject` / `f:reifiesObject` estimates only, capped by the base-edge count: `f:reifiesPredicate`'s bound-object estimate splits the arena evenly across predicates, which put `TREATS` at 31k reifiers when it has 1.1M and flipped the choice the wrong way at full scale, while the endpoint lookups only get small when a binding makes them small — a child that already binds the reifier makes the chain one probe per driving row, which no sweep beats (P22 / C7 / C10 regressed 200× when the arena probe was taken there).
