...
When eligible, the provider walks the base index via per-predicate PSOT scans (PSOT-direction sidesteps a SPOT quirk where constant blank-node subjects don't return rows reliably across all backends), groups results by annotation SID, decodes each bundle into an `EdgeKey`, and returns `AttachmentEventCoverage::Authoritative`. The indexer then seals an authoritative arena from the events. Any subsequent indexer pass coerces `had_annotation_arena = true`, so the same ledger is no longer bootstrap-eligible — defensive drops carry the bit forward and stay in the M2a scan-fallback state.

A later reindex does **not** recover that state on its own. The bit is set by every indexer-produced root with `has_annotations = true` — `IncrementalRootBuilder::build()` and `root_assembly.rs`'s `encode_and_write_root_v6` both coerce it — so once set, no pass clears it and the ledger is never bootstrap-eligible again. `fluree reindex --rebuild-annotations --force` is the way past the gate; it rebuilds the arena from currently-live annotations.

The conservatism is not removable. A rebuild from live bundles reproduces a never-sealed ledger's arena exactly, but it cannot reproduce one whose arena was sealed and then dropped, because the dropped arena's retract events are gone: a retracted `f:reifies*` row leaves no trace in the index — not even an `op=false` row — so no code can tell the two cases apart. That is why `--rebuild-annotations` requires an explicit acknowledgement rather than detecting when it is safe.
