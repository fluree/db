...
The CLI (`fluree bm25 create/list/sync/drop`) drives a server when one is reachable, and runs in-process against local storage otherwise — so it also works under `docker exec` against a running server's data directory; see [CLI Management](#cli-management) below.
...
Incremental sync uses property dependency tracking to identify which subjects changed since the last indexed commit, then re-runs the indexing query and keeps the rows belonging to those subjects. That is only sound when the tracking is complete, so each sync first checks the query's shape: an object-form `select` naming one document variable, explicit properties (no `*`, no nested projections, no reverse selections), and `where` patterns all rooted at that variable, with no variable predicates, `@reverse` terms, `@path` aliases, nested patterns on other subjects, subqueries, `depth`, or result-shaping clauses (`limit`, `offset`, `groupBy`, `having`, `selectOne`). A query outside that shape is rebuilt in full on every sync, and the decline is logged at `warn` with the reason — narrow the query to the documented shape to get incremental syncs back.

Within that shape, a window whose commits touch none of the tracked properties advances the watermark and persists it without touching the index. It does not rebuild: the tracked set is total, so an empty change set means the index is already correct as of the ledger's head. (Earlier releases rebuilt the whole index in that case, which on a ledger whose writes mostly miss the indexed properties made the cheapest possible window trigger the most expensive possible operation on every commit.)

Where the query shape allows it, the affected subjects are bound into the query itself as a `values` clause, so the scan seeks to those subjects instead of walking the corpus. That makes the indexed portion of a sync flat in corpus size rather than linear. Novelty — rows committed since the last index build — has no subject index and is still walked, so the win over that portion is small; `reindex_min_bytes` is what bounds how much novelty a sync has to walk, and the two are complementary.

Narrowing is skipped, and the whole-ledger scan used, whenever the indexing query is not one that can be narrowed safely: a list-form `select` (which does not say which variable is the document), a `select` binding more than one variable, a query that already carries its own top-level `values`, or any clause that shapes the result set rather than the scan — `limit`, `offset`, `groupBy`, `having`, `selectOne`. Those are declined because narrowing would change *which* rows come back, not just how they are found, and the sync treats a subject the query did not return as one to remove. Falling back is always correct, only slower. Each decline logs at `debug` with the condition that caused it.

> **Upgrading an existing index.** Indexes built before document identifiers were canonicalised can hold two keys for one document — the full IRI written at build time and a prefixed form written by a later sync. **Run one `resync_bm25_index` after upgrading.** An incremental sync will not repair it: the removal pass only visits subjects the commit affected, so it never reaches the orphaned key, and document counts stay inflated until a full resync enumerates the existing keys and rewrites them.
...
- Bounded concurrency for concurrent sync operations, and at most one sync in flight per index
- Registers/unregisters graph sources dynamically

Budget for it before turning it on over a large corpus. Each sync re-runs the indexing query (see [Syncing](#syncing)). Where that query can be narrowed to the affected subjects the indexed portion no longer scales with the corpus, which is what makes a write-heavy source ledger affordable per debounce window. Where it cannot — any of the declined shapes listed above — each sync is still a full indexing-query execution, and debouncing bounds how often that happens rather than what each one costs. Check the `debug` log for the decline reason if syncs stay expensive; at a few million documents on a query that cannot be narrowed, explicit syncs from a scheduled maintenance job keep that work on a cadence you choose.
...
The `fluree bm25` command group manages indexes without writing Rust. Each command runs against a server when one is reachable — `--remote <name>` for a configured remote, or a locally-running server automatically — and in-process against local storage otherwise. Pass `--direct` to force the in-process route, which works under `docker exec` against a running server's data directory (native storage uses per-file locks, so this is safe alongside a running server). See [fluree bm25](../cli/bm25.md) for the full command reference.
