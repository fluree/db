...
Incremental sync uses property dependency tracking to identify which subjects changed since the last indexed commit, then re-runs the indexing query and keeps the rows belonging to those subjects. If no affected subjects are detected, it falls back to a full resync.
