...
Incremental sync uses property dependency tracking to identify which subjects changed since the last indexed commit, then re-runs the indexing query and keeps the rows belonging to those subjects. Only affected documents are re-indexed, but the query that finds them runs over the whole source ledger — so a sync costs about the same whether one document changed or a thousand. Sync cost scales with corpus size, not with the size of the commit. If no affected subjects are detected, it falls back to a full resync.
...
Budget for it before turning it on over a large corpus. Each sync re-runs the indexing query over the entire source ledger (see [Syncing](#syncing)), so a write-heavy source ledger runs a full indexing-query execution every debounce window. Debouncing bounds how often that happens, not what each one costs, so the bill tracks corpus size rather than commit size. At a few million documents, explicit syncs from a scheduled maintenance job keep that work on a cadence you choose.
