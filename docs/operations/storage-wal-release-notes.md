...
- WAL segments use the `FRDOSEG2` format with an xxh64 checksum over the
  payload followed by the frame header. Unknown segment formats are rejected
  and retained on disk. WAL is new in this release; unpublished development
...
- Before downgrading file storage, recover with the current binary, stop it
  cleanly, and confirm its WAL segments have been retired. Binaries from before
  this release do not recover WAL segments. Skipping recovery can let a later
  WAL replay overwrite or delete changes made by the older binary.
