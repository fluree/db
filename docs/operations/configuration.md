...
| `FLUREE_ICEBERG_PREDICATE_PUSHDOWN` | on | Toggles pushing simple date/int/bool FILTER comparisons into the Iceberg reader for **row-group pruning** (skip groups whose min/max rule out the predicate) and, on the Arrow reader, **exact row filtering** (drop non-matching rows during decode). The in-engine FILTER remains the authority, so this only ever affects performance. Set to `0`/`false`/`off` to disable both. |
