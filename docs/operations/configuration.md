...
| `FLUREE_R2RML_SUBJECT_KEY_PUSHDOWN` | **off** | Toggles pushing a bound (constant) subject down to the Iceberg scan. When a query names a specific subject IRI, Fluree reverses the `rr:subjectMap` template to recover the key column value and pushes it as an equality predicate for row-group pruning + row filtering, instead of scanning the whole table. Only unambiguously-reversible template shapes and integer/string key columns are pushed; everything else falls back to a full scan. The mapping's subject equality is always re-checked in-engine, so this only affects performance. **Off by default** pending live validation on production backends; set to `1`/`true`/`on` to enable. Requires that query subject IRIs use the same percent-encoding the mapping generates — see [R2RML › Bound-subject key pushdown](../graph-sources/r2rml.md#bound-subject-key-pushdown). |
