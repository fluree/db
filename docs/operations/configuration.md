...
| `FLUREE_STORAGE_FSYNC` | on | File-storage durability. On (the default), a write is reported complete once it is in the storage root's WAL and that log is flushed, so an acknowledged commit survives power loss at no more than one flush per commit, shared between commits that overlap; the files themselves are flushed in the background and replayed on start. Set to `sync` to flush each file and its directory entry on the write path instead, as releases before v4.3 did. Set to `0`/`false`/`off`/`no` to report completion once the bytes reach the OS page cache — faster, but a power loss or kernel panic can lose acknowledged commits. Read once per storage construction, and **overrides** a storage node's `durability` property so a one-off run needs no config edit. Applies only to the local file backend; S3 acknowledges after replication and the Raft log flushes independently. Raft nodes each journal the shared payload root under a log of their own, flushed on every write. Derived content (index nodes, dictionaries, sketches, arenas) is written page-cache in every setting, since it is recomputable from the commit chain. See [Storage durability](storage.md#durability). |
