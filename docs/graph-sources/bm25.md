...
`fluree-search-httpd` only **serves** queries against existing indexes; it does not create them. A BM25 index is created by the main Fluree server (`POST /v1/fluree/bm25/create`, see [API endpoints](../api/endpoints.md#graph-source-endpoints)), by the `fluree bm25` CLI, or through the Rust API (`Bm25CreateConfig` + `create_full_text_index`). Vector indexes have no HTTP endpoint yet and are still created via the Rust API (`VectorCreateConfig` + `create_vector_index`).

The recommended workflow is:

1. Create the BM25 / vector index on a shared storage path — via the main server, the CLI, or the Rust API.
