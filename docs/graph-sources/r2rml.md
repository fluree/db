...
This is **opt-in** while it is validated against production backends. Enable it by setting `FLUREE_R2RML_SUBJECT_KEY_PUSHDOWN=1` (see [Configuration](../operations/configuration.md)). With it off (the default), bound-subject queries are still answered correctly by scanning and filtering in-engine.
