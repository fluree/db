...
Explain on a policy-scoped view (any view that is not root, including a plain `default-allow: false` narrowing) returns a plan built without ledger or annotation statistics, and `plan.reason` says the statistics are hidden from the explain by policy while execution still plans with them. A caller who can only see a filtered subset must not learn unfiltered cardinalities from the planner. Unrestricted explains retain their statistics.
