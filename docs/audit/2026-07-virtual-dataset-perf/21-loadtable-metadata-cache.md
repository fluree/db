...
> **SCOPE CORRECTION (2026-07-15, measured via F17's baseline — see register [[F21]]).** The "warm-disk first-ask = zero loadTable GET" claim above holds ONLY inside the pointer's TTL window. As shipped in #1503 the pointer TTL defaults to **300 s** and `get_metadata_location` **prunes the entry on an expired read** — so the pointer spares the GET only for a table touched **within ~5 min of its last write**. A genuine cold first-ask after a quiet gap, or a small dim query late in a long corpus run whose table's pointer has since expired, **still pays the GET**. Observed: only one `.pointer.json` (FACT_WEB_EVENT) survived on disk while dim queries (q004/q024/q043) did GETs every rep; on a slow-catalog day those ~1.5 s GETs blew the small queries' perf budgets. So #1503's real-world win is an **intra-window / intra-session** amortization, not a universal cold-first-ask elimination. Whether to widen the window (longer TTL = more staleness vs stale-serve+revalidate = a correctness question) is F21's design question — through the sketch gate, not decided here.
...
> **⚠️ WIRING OBLIGATION (for whoever implements Iceberg snapshot time-travel).** The `as_of_t` rider is a FORWARD-GUARD: `disk_catalog_cache::pointer_is_usable` already refuses a pointer whose cached `snapshot_ms` is older than a requested `min_snapshot_ms` (hermetic `pointer_ttl_and_as_of_rider`), but the caller in `load_table_context` currently passes `None` (a latest-snapshot read) because Iceberg time-travel is deliberately unimplemented on the scan path (`GREP: r2rml-as-of-t`, `r2rml.rs`). **When time-travel lands, you MUST thread the requested snapshot's `timestamp_ms` into `get_metadata_location(&lt_key, Some(requested_ms))`** — otherwise bounded pointer staleness could silently answer a time-travel query from an older snapshot than requested. The guard exists and is tested; only the caller argument is inert. `grep r2rml-as-of-t`.

**Implemented** as `perf/r2rml-loadtable-meta-cache` (stacked on #1502): the pointer + eager-REST-client/deferred-GET `LazyS3Storage` split (the OAuth token exchange rides the first catalog op, so deferring the GET zeroes both gate counters); switch `FLUREE_ICEBERG_LOADTABLE_PTR_CACHE` (default on) + TTL `FLUREE_ICEBERG_LOADTABLE_PTR_TTL_SECS` (300 s, 0 = off). Lib + R2RML integration suites green; the `LazyS3Storage` is `Clone` (Arc-shared cell) because `SendParquetReader` requires `S: Clone` — clones share one build, so N range reads still force one loadTable GET.
