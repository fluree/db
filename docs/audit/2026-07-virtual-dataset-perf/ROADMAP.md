...
|---|---:|---|---|---|
| **q031** | **72.1 s** | `scan_table.n=1448`, `load_table.n=7` (21 s); `files_selected=0` (dim lookups, no scan_plan) | **REGRESSION.** PR-8b's query-scoped parent-memo (present; unit tests pass) is **not reached** by q031's plan → 1448 un-memoized DIM_PRODUCT re-scans (the pre-PR-8b 1306 shape). `FILTER(?oh<?rp)` inner-join + `LIMIT`. **A/B: `FLUREE_R2RML_LIMIT_PUSHDOWN=0` → scan_table 1448→8** (memo engages; but 88 s, LIMIT no longer cuts the fact scan). REFUTED: BATCHED_OPTIONAL(_STAR) (PR-4c), PARENT_MEMO, NUMERIC_STATS (PR-7), TOPK, STAR_TM_PRUNE, FILTER_CONSUMPTION, SCAN_CACHE — none revert it. Only post-PR-8b unswitched operator change = **PR-5 GraphOperator inner-subplan forwarding refactor (51aafc78f)** → leading culprit; PR-4a limit-pushdown was already present at PR-8b's 188 ms gate. **Confirm via bisect (build @ 2a07bbbc4 pre-PR-5, run q031).** | make the query-scoped parent-memo engage on the LIMIT-pushdown batched driving path (restore PR-8b "survives operator rebuild" for the limit-batched path). New F-number; potentially indicts shipped PR-5 #1495. |
...
**Slate for AJ (design-first PRs, in impact order):** (1) **q031 memo-on-limit-path** regression — confirm PR-5 via bisect, then restore memo engagement (biggest single win, 72 s→~0.2 s, and it's a shipped-PR regression). (2) **PR-4d** batched-OPTIONAL whole-seed (q016 43 s→~1 s, +q050-class, and overlaps q029's re-drive family). (3) **F17 UNION budget forwarding** (q029 125 s→secs). (4) **q038 fused-COUNT** (48 s→secs). (5) **q041 ref-IN pushdown** + (6) **q053/q017 semi-join** (13 s→secs; borderline q028/q017 likely fall out). **Expected end-state:** all 50 ok-queries ≤ ~3 s. No engine work started — this slate goes to AJ first.
