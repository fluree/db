...
| **q029** | 125 s (pre-slate) → **2.61 s** (3-rep in-protocol, F17+cache stack) | `scan_table.n` 253→**2**, decodes **1.94 M→7.7 K** (2 sweeps→1); cold isolated first-ask ~57 s-class | **F17 SHIPPED** — UNION *and* BIND absorbed the LIMIT budget (two absorb points; branch root is `BIND(const AS ?et)`). Forwarding kills the 253× re-drive (→ 2 sweeps); the budget-met **lever** skips branch-2 (→ 1 sweep). The union is **sequential** (branch-1 fully drained before branch-2 built), so the lever removes one whole sequential sweep. **NOT byte-identical** — post-decode `eventType` residual + geometric window ⇒ a *different valid* first-100 (`rows_only`, like q055/q057); oracle-parity 0-mismatch. | **Residual = ONE full FACT_WEB_EVENT sweep, S3-FETCH-bound** (decode trivial 0.57 s; the wall is `iceberg.fetch_bytes`). **Wall model: ~7 s fixed (loadTable + oauth + scan_plan) + N_sweeps × one-sweep-fetch.** Same PR-2a shared floor as q016/q031/q056 — but **fetch-rate-sensitive, not decode** (so PR-2a-class work must cut file-count / S3 fetch, not decode speed). Switch `FLUREE_R2RML_UNION_BUDGET`; own PR stacked on #1506. |
...
| **q016** | 43.1 s → **17.47 s (PR-4d #1501, MEASURED 3-rep in-protocol)** | pre `scan_table.n=182`, 1.4 M reads → **post `scan_table.n=3`, 23,010 reads** | **F14** — batched-OPTIONAL was per-OUTER-batch (`optional.rs:2029/2039`); the outer FACT_ORDER emitted ~180 budget-windowed batches, each driving a fresh FACT_SHIPMENT inner scan (182 = 2 outer + 180 inner). **PR-4d SHIPPED** (`19-...md`): coalesce the whole driving side into ONE seed → **scan 182→3, 2.5× wall** (oracle hash-clean). | **PR-4d gated on scan-count 182→3 + parity (MET).** **Post-4d 17.47 s decomposition (honest, spans nest — fetch/decode are inside parquet_read, not additive):** `load_table` 1.55 s (n=1; 1 table amortized in-corpus) + `parquet_read` 4.77 s (n=23,010 = 3× full 7,670-file fact reads, ~1.6 s/scan — the ~1 s floor roughly holds) + **~11 s hash-left-join partition/materialize** of the 180 K-row FACT_SHIPMENT inner into 5000 order buckets (the un-attributed residual = the combine/LIMIT of 5000 wide fact-to-fact rows). **≤3 s is NOT closed by the current slate:** the loadTable-cache (item 7) shaves only ~1.55 s; the residual is 3 fact-scale reads (**PR-2a decode-wall**) + the ~11 s join/materialize (**needs its own follow-on** — a scan-count/materialization lever, e.g. IN-set probe of FACT_SHIPMENT keyed by the 5000 `?o` instead of a full 180 K-row inner scan+partition). q016 likely lands ~10–15 s even after item 7; its ≤3 s path is a NEW item, hypothesized not measured. |
...
**Slate for AJ (design-first PRs, in impact order — all end-state figures are the cache-thrashed full-corpus bar, not warm-only; AJ SIGNED 2026-07-14):** (1) **q031 RESOLUTION-PRUNE (F20)** — propagate the `edw:product` RefObjectMap target (DIM_PRODUCT) to constrain the `?p edw:name` resolution, killing the 6-dim fan-out (`load_table` 7→2, ~15 s + the fan-out re-scans; **72 s → ~3–5 s cache-thrashed, loadTable-bound** — measured, §F20 sketch). Deterministic gate = `load_table.n` 7→2 + scan collapse; wall REPORTED not gated. Standalone, own kill switch, join-var-provenance soundness invariant. **Necessary but NOT sufficient for the ≤3 s bar** — the residual ~2–3 s is the 2 loadTable credential GETs, closed by item (7). **Supersedes the F18 cold-floor framing.** (2) **PR-4d** batched-OPTIONAL whole-seed (q016 scan-count 182→single-digit; SHIPPED #1501 pending baseline). (3) **F17 UNION budget forwarding** (q029 125 s → low-single-digit s; distinct operator from PR-4d). (4) **q038 fused-COUNT** (48 s → sub-second/low-s). (5) **q041 ref-IN pushdown** + (6) **q053/q017 semi-join** (13 s → low-single-digit s; borderline q028/q017 likely fall out). (7) **persistent loadTable-metadata cache** (below) — the ≤3 s closer for the loadTable-bound tail. **Expected end-state:** all 50 ok-queries ≤ ~3 s cache-thrashed, contingent on (7) for the loadTable-bound queries.

**Item (7) — persistent loadTable-METADATA cache (PR-8 slice-2 extension; the named ≤3 s closer).** The post-plan-shape residual on the tail is the **loadTable credential/REST GET** (~1–3 s/table), which PR-8 slice 2 explicitly does NOT persist (creds are per-process). **HARD SECURITY CONSTRAINT (AJ standing ruling — stated now so the sketch cannot walk into it): vended credentials are NEVER persisted to disk.** The persistable artifact is the loadTable **metadata response SANS credentials**, content-addressed exactly like slice 2 (metadata_location = S3 content hash). Credentials are fetched **per-process, in-memory, ONLY when an actual S3 fetch is needed** — and when the parquet is served from the disk artifact cache (the steady-state cache-thrashed case), **zero creds and zero REST round-trips are needed at all**, which IS the first-ask win. Beneficiaries (quantify from the 4d full-corpus baseline): q031's last ~2–3 s (2 loads), plus likely chunks of q016/q038/q029 wherever `load_table.n > 0` in the baseline. **Sketch AFTER F20 ships** (design-first); this entry reserves the item + pins the security invariant.

**Shared cold-floor observation (2026-07-14; REVISED by the PR-4d baseline measurement).** After the plan-shape fixes, several tail queries bottom out on a shared residual set: **full 7,670-tiny-file fact scans + a hash-join/materialize** (FACT_INVENTORY for q031-post-F20; FACT_SHIPMENT for q016-post-PR-4d; the fact tables are all 7,670 files / ~180–300 K rows / ~45–55 MB — file-count-bound, not byte-bound). A single 7,670-file scan is ~1–1.6 s (q018 1.06 s, q044 0.96 s; q016's 3 scans measured ~1.6 s each). **The earlier optimism — "plan-shape fixes land ≤3 s without PR-2a" — is REFUTED for q016 by measurement:** q016 post-4d is **17.47 s**, of which only ~6 s is loads+scans and **~11 s is the hash-left-join partition/materialize** (a full 180 K-row inner scan + 5000-bucket combine). So the ≤3 s bar for the fact-JOIN tail needs BOTH **PR-2a** (the per-file decode-wall, for the multiple 7,670-file scans) AND a **materialization/scan-count lever** (IN-set probe instead of a full inner scan+partition) — not just the loadTable-cache. **PR-2a is no longer "stays closed" for these — it is on the path for q016 (and likely q053/q041 fact-scan-bound tails).** The single-loadTable-bound queries (q031-post-F20) are the exception where item (7) alone gets close. First-EVER-S3-touch (~197 s for 7,670 files) remains reported, not gated (disk artifact cache is steady-state per AJ's bar).
...
## EXPLORATION FAMILY (q055-q059) — naive first-touch profiling *(class: exploration; OUT of the north-star accounting)*

**Added 2026-07-15; kept OUT of the north-star 44/51 accounting pending AJ's ruling on whether these join the ≤ 3 s bar.** These are the queries every human or LLM agent runs *first* on an unknown dataset — peek, size, schema, census, histogram — the maximum-resolution-fan-out case (all 16 TriplesMaps, no constraint to prune). The BI-derived corpus had zero pure-wildcard coverage.

Measured 2026-07-15 (virtual-sf01 vs native-sf01, hot cache, 1 rep, HEAD 6750286df with the #1503 loadTable-cache). **All five complete `ok` — none DNF** (the predicted DNFs for q056/q058/q059 were overturned by measurement). The honest framing is **"all-complete-but-slow": the predicate-less materialization is the real first-touch cost (168 s), not a hard failure.**

| q | query | native | virtual | rows | gate | mechanism |
|---|---|---:|---:|---:|---|---|
| q055 | `?s ?p ?o LIMIT 5` | 1 ms | 2.44 s | 5 | rows_only | LIMIT does not prune the scan; the engine reads one full fact table (~7,679 files, 63 MB) then truncates to 5. Near the bar. |
| **q056** | `COUNT(*){?s ?p ?o}` | 0 ms | **168.29 s** | 1 | full | **the headline gap.** Predicate-less COUNT — no manifest COUNT shortcut (not a single resolved table). Materializes *every triple*: 69,046 files across all 16 tables, 492 MB, full object-column fetch (`fetch_bytes` 2,153 s summed = **63× q057** over the same file set). The un-fused COUNT reads object values it only needs to count — same family as q038's un-fused COUNT. **Measured 168.3 s hot; `timeout_s` 300 is CI-stability headroom (168/180 = 93% is fragile), not an expectation.** |
| q057 | `DISTINCT ?p LIMIT 100` | 0.9 s | 34.93 s | 100 | rows_only | same 69,046-file breadth, but object columns pruned (only predicate enumeration). Bottlenecks on per-file footer/decode overhead. `LIMIT 100` truncates a ~120-distinct set (119 `rr:predicate` + `rdf:type`) → nondeterministic → rows_only (empirically the sole parity failure). |
| q058 | `?s a ?type` census GROUP BY | 0.15 s | 5.40 s | 16 | full | census needs only subject-key + per-table `rr:class` constant → **16 scans (one per TriplesMap)**, column-pruned. The cheapest wildcard, and why the DNF prediction was wrong: per-TM counting engages, so there is no whole-dataset object fetch. |
| q059 | predicate histogram GROUP BY ?p | 2.6 s | 32.38 s | 10 | full | same object-pruned 69,046-file breadth as q057, plus GROUP BY/ORDER BY (cheap). Deterministic (`ORDER BY DESC(?n) ?p` tiebreak) → full hash. |

**Mechanisms.** (1) **Whole-dataset breadth (~69,046 files) = PR-2a at maximum fan-out** — every unconstrained `?s ?p ?o` touches every file of every table, so the per-file decode-wall (PR-2a, the master lever) is the shared floor here across all 16 tables at once. (2) **Object materialization is the q056 tax** — the 63× `fetch_bytes` gap between q056 (full object fetch, 168 s) and q057/q059 (object-pruned, 32-35 s) over the *same* file set shows the un-fused predicate-less COUNT materializes object values it only counts; a fused predicate-less `COUNT(*)` (Σ over TriplesMaps of rows × POMs from manifest record counts) would take q056 to sub-second — the single highest-value exploration fix (PR-6 family, q038 sibling). (3) **Column pruning already works** for `?p`-only and subject-key-only shapes (q057/q058/q059 prune object columns), which is why only q056 is catastrophic.

**Follow-on candidates (impact order, no burndown rows — exploration is out of the north-star count):** (a) predicate-less fused-`COUNT(*)` for q056 (168 s → sub-second; PR-6 family); (b) PR-2a per-file decode-wall lowers the whole family's floor; (c) q058's 5.4 s census is already acceptable, q055 near the bar.

**Timeout-headroom observation (no action).** q056's 300 s gate over a 168 s wall is the same fragility class as **q029's 180 s gate over its ~150 s wall (also a ~93% ratio)** — when F17 lands and re-measures q029, it likely deserves the same headroom treatment. A gate deadline is a CI-stability parameter, not the measurement; the wall is recorded honestly in the row above either way.
