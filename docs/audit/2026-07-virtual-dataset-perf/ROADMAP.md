...
**Closure criterion (AJ):** every corpus query (excl. the 3 expected-error) hot ≤ low single-digit seconds, 3-rep protocol, no DNFs at manifest timeouts. **Clean baseline** (priming + measured 3-rep hot, per-query manifest `timeout_s`, post-PAT-swap, healthy net) supersedes the contaminated `pf9_virt_on.jsonl`. **42 of 50 ok-queries are already ≤ 3 s.** Top-k healthy (q046 125 ms, 10/7660 pruned), q019 pruning intact (792 ms, 7670 pruned), q012 fine via fallback (491 ms). The **8-query tail** (>3 s hot) and its mechanism + remedy:

| q | hot | signature (spans) | mechanism | remedy (mirrors) |
|---|---:|---|---|---|
| **q031** | **72.1 s** | `scan_table.n=1448`, `load_table.n=7` (21 s); `files_selected=0` (dim lookups, no scan_plan) | **PRE-EXISTING, NOT a regression (F18).** PR-8b's query-scoped parent-memo (present; unit tests pass) is **not reached** by q031's plan → 1448 un-memoized DIM_PRODUCT re-scans on the `FILTER(?oh<?rp)` inner-join + `LIMIT`. **A/B: `FLUREE_R2RML_LIMIT_PUSHDOWN=0` → scan_table 1448→8** (memo engages; but 88 s — LIMIT no longer cuts the fact scan). Bisect + gate forensics EXONERATE every PR: A@2a07bbbc4 and B@b1cb988c2 both = 1448/~70 s, and PR-8b's OWN gate recorded q031 at **65 s / scan_table 1448** (`pf8b_virtual.jsonl`) — identical to now. The "188 ms PR-8b sentinel" was a fully-warm reps=3 artifact (q029, 125 s cold, ran 156 ms in that same phase). So the 1448-scan plan is unchanged since PR-8b; q031 is **cache-sensitive** (188 ms fully-warm ↔ 65–72 s cache-thrashed). | make the query-scoped parent-memo engage on the LIMIT-pushdown batched driving path so you get BOTH the memo (8 scans, not 1448) AND the LIMIT cut on the fact scan. **Expected end-state: low-single-digit seconds cache-thrashed / sub-second warm** (partial LIMIT-cut fact scan + one memoized dim load — meets the ≤~3 s bar; NOT 0.2 s, which was the warm-only artifact). File as F18 (perf, memo × limit-pushdown), + a q031-shaped hermetic differential. |
| **q029** | 125 s | `scan_table.n=253`, `files_selected=15340`, 1.94 M reads | **F17** — UNION absorbs the LIMIT budget; both FACT_WEB_EVENT branch scans re-driven ~253×. | UNION budget forwarding (mechanism-class D; mirror PR-5 wrapper-forwarding). Likely subsumed by PR-4d re-scan family. |
...
| **q016** | 43.1 s | `scan_table.n=182`, 1.4 M reads | **F14** — batched-OPTIONAL drives the seed in windows and re-scans the inner (FACT_SHIPMENT) per window. | **PR-4d** whole-seed single inner scan + in-memory hash-join. |
...
**Slate for AJ (design-first PRs, in impact order — all end-state figures are the cache-thrashed full-corpus bar, not warm-only):** (1) **q031 memo-on-limit-path (F18)** — make the parent-memo engage with LIMIT pushdown (biggest single win, 72 s → low-single-digit s cache-thrashed; **pre-existing, not a regression** — all PRs exonerated). (2) **PR-4d** batched-OPTIONAL whole-seed (q016 43 s → low-single-digit s, +q050-class, overlaps q029's re-drive family). (3) **F17 UNION budget forwarding** (q029 125 s → low-single-digit s). (4) **q038 fused-COUNT** (48 s → sub-second/low-s). (5) **q041 ref-IN pushdown** + (6) **q053/q017 semi-join** (13 s → low-single-digit s; borderline q028/q017 likely fall out). **Expected end-state:** all 50 ok-queries ≤ ~3 s cache-thrashed. No engine work started — this slate goes to AJ first.
