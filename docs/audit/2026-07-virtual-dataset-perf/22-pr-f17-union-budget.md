...
**Secondary lever — the union runs all branches before the budget is met [⚠️ CORRECTED 2026-07-15: this DOES affect q029, contra the original "nil for q029"].** The union's branch loop (union.rs:299) runs *every* branch for an input row before returning, so once branch-1 fills the budget, branch-2 still scans fully. For q029 (1 unit-seed input row, 2 branches) that means **branch-2 decodes a whole extra 7,670-file sweep after branch-1 already supplied 100** — measured as the 2nd of the two sweeps in the 45.9 s. The lever — break the union's input/branch loop once `pending_output_rows >= row_budget` (and for a correlated union, the per-`(row,branch)` cap of `k` also bounds the multi-input case) — would skip branch-2 → ~1 sweep ≈ **~23 s**. Still bottoms out on the 1-sweep PR-2a floor. Originally proposed out-of-scope; the measurement makes it a live ship-vs-follow-up decision for the lead.
