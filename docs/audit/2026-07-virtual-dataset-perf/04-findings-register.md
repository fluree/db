...
**The only true resetter — and the one SOUND residual fix (DEFERRED, no corpus impact).** The single `&self`-derivation that re-creates a fresh memo is `with_graph_ref` (`context.rs:1222`), used ONLY by `DefaultGraphSourceOperator` (edge-annotation multi-source) and SERVICE — **not on q031's path, not exercised by any corpus query.** Its reset is deliberate FOR `const_sid_cache` (that ctor switches to a different graph's store, so a store-implicit const→s_id cache MUST NOT alias). But the **parent-memo key is store-disambiguated** — `(graph_source_id, parent_tm, cols, as_of_t)` (`operator.rs:75`) — so cloning `r2rml_parent_memo` there (while keeping `const_sid_cache` fresh) is SOUND. It would only ever help a correlated `RefObjectMap`-parent scan rebuilt across a SERVICE / multi-source-default R2RML boundary. **Filed as the residual F19 fix; DEFERRED — it rides the next PR that touches that subsystem (the loadTable-cache slate item is the natural vehicle) with its own hermetic `with_graph_ref`-derivation memo test. It is NOT in the F20 PR** (no corpus query would move).
