...
**Design & implementation (2026-07-15, sketch `22-pr-f17-union-budget.md`, own PR stacked on #1506; lead-approved).** The mechanism has **two absorb points**, not one: the `LIMIT` budget dies at both `UnionOperator` AND `BindOperator` (q029's branch root is `Bind("…" AS ?et)`), each inheriting the trait's no-op `set_row_budget`. Verified plan: `Limit→Project→Union→[per branch] Bind→R2rmlScan[eventType=const fused scan constraint]` — the `eventType` equality is a fused scan constraint (`rewrite.rs:273-315`, `operator.rs:623`), not a `FilterOperator`, so the scan's `emitted` counts matches and a `row_budget=k` caps its materialize window (`operator.rs:1274`; the default window is 512K rows = the whole table, so a union-drain cap is insufficient — the budget must reach the scan). **Remedy = forward the budget through both**: `Union` stores it and re-applies to *each* branch build (full `k`, not `k/n` — a single branch may supply all `k`); `Bind` forwards to its child (reclassified from absorb to forward — categorically sound, 1:1/order-preserving). Every decline case (ORDER BY / DISTINCT above, in-branch FILTER/reorder) is handled by the **existing** absorb boundary — no new decline predicate. Switch `FLUREE_R2RML_UNION_BUDGET` (default on). Corpus blast radius = q029 only (q042 is UNION-without-LIMIT; q029 is the sole BIND-under-LIMIT). **NOT subsumed by PR-4d** — different operator (`UnionOperator` vs batched OPTIONAL); PR-4d does not touch UNION.

**MEASURED (2026-07-15 live gate `pf_f17_gate.log`) — two design predictions corrected.** Virtual q029 **166.7 s → 45.9 s (3.6×)**; parquet decodes **1,940,510 → 15,406 (126×)**; bytes **18.8 GB → 150 MB**; `scan_table` 253 → 4; oracle-parity green (`rows_only`). **(1) Byte-identical REFUTED:** ON hash ≠ OFF hash. `eventType` is a *post-decode* residual (constant-object `consumed_filter`, operator.rs:1679-1688) — cannot skip files — and the scan window is geometric-growth (operator.rs:1382), so the budget materializes a *different valid* first-100 (`rows_only`, same class as q055/q057), and each branch still decodes a **full 7,670-file sweep** to accumulate 100 rare matches. The win is **re-drive elimination, NOT file-level early-stop**; the residual (45.9 s ≈ 15,406 tiny-file decodes) = the **PR-2a per-file floor**, shared with q016/q031/q056. The budget bounds OUTPUT rows; a low-yield residual is only met after a near-full INPUT scan. **(2) Secondary lever CORRECTED — it DOES help q029** (the earlier "no corpus query needs it" was wrong): the union runs both branches before the budget is met (union.rs:299), so branch-2 sweeps fully after branch-1 already supplied 100. Breaking the union loop at `pending_output_rows >= row_budget` skips branch-2 → ~1 sweep ≈ **~23 s** (~2× more; still the 1-sweep PR-2a floor). q029 is a burndown row at **~46 s (as shipped)** / ~23 s (with the lever); low-seconds needs PR-2a. Full design + corrections: `22-pr-f17-union-budget.md`.

**Fix owner.** engine (UNION + BIND budget forwarding — implemented) + harness (per-query manifest timeout).
...
## F19 — PR-8b query-scoped parent-memo across correlated sub-executions — MECHANISM REFUTED (the memo already shares on q031's path); residual sound fix filed, no corpus query affected *(closed; the FIFTH flip in the q031 chain)*

**CORRECTION (2026-07-14, confirm-experiment before implementing the fix — the SUSPECTED CAUSE below is REFUTED; kept as the 5th link in the q031 verdict chain).** The "base `ExecutionContext` ctors re-create a FRESH `::default()` memo → the correlated inner-join loses it" reading is wrong, on two independent counts:
1. **The named sites are ROOT constructors, not derivations.** `context.rs` 337/392/451/499/546/595 are `new` / `from_graph_db_ref[_with_from_t]` / `with_time` / `with_overlay` / `with_time_and_overlay` — they take `snapshot`+`vars` (no `&self` parent) and run ONCE at query top (the live site is `runner.rs:786` `with_time_and_overlay`). There is **no parent memo Arc to clone** at those sites; a fresh memo there is correct. The clone they were contrasted with (1103/1161) belongs to the `&self`-deriving `with_active_graph`/`with_default_graph`.
2. **q031's real per-batch derivation ALREADY shares the memo.** `rewrite_patterns_for_r2rml` is called ONLY inside `GraphOperator` (`graph.rs:288`/`:498`), so a virtual query's R2RML scan always runs under a `GraphOperator`, whose per-batch inner ctx is derived by `with_active_graph` — which **clones** `r2rml_parent_memo` (`:1103`). Two hermetic experiments (in `pr8b`, run then reverted) prove it: deriving the ctx per rebuild via `with_active_graph` ⇒ parent scanned **1×** (memo Arc shared); via a genuinely fresh `ExecutionContext::new` ⇒ parent scanned **5×** (control — the harness DOES detect memo-loss). The existing `parent_lookup_survives_operator_rebuild` already passes with one ctx; a "memo hits across N driving batches" test would pass with or without the base-ctor change → false confidence.

**Why the symptom was never memo-inertness.** q031's 1448 (post-F20: 243) scans are NOT parent lookups — `?p edw:name` maps to a COLUMN, not a `RefObjectMap`, so the parent-memo never applied to them; that count is the F20 shared-predicate fan-out. The one `RefObjectMap` parent lookup (fact `edw:product` → DIM_PRODUCT) sits in the driving fact scan and is already deduped by the per-operator cache. So `PARENT_MEMO` on/off being identical for q031 is an artifact of q031's SHAPE, not evidence the memo is inert. It is not inert.

**The only true resetter — and the one SOUND residual fix (DEFERRED, no corpus impact).** The single `&self`-derivation that re-creates a fresh memo is `with_graph_ref` (`context.rs:1222`), used ONLY by `DefaultGraphSourceOperator` (edge-annotation multi-source) and SERVICE — **not on q031's path, not exercised by any corpus query.** Its reset is deliberate FOR `const_sid_cache` (that ctor switches to a different graph's store, so a store-implicit const→s_id cache MUST NOT alias). But the **parent-memo key is store-disambiguated** — `(graph_source_id, parent_tm, cols, as_of_t)` (`operator.rs:75`) — so cloning `r2rml_parent_memo` there (while keeping `const_sid_cache` fresh) is SOUND. It would only ever help a correlated `RefObjectMap`-parent scan rebuilt across a SERVICE / multi-source-default R2RML boundary. **FIXED (2026-07-15, riding the loadTable-cache PR as its own labeled commit):** `with_graph_ref` now clones `self.r2rml_parent_memo` (keeping `const_sid_cache` fresh); hermetic `pr8b::parent_memo_survives_with_graph_ref_rebuild` proves the DIM parent is scanned once across 5 `with_graph_ref` rebuilds (the `with_active_graph`-only coverage missed this path). No corpus query moves (none exercises SERVICE / multi-source-default R2RML), so it ships gate-neutral.

**WIRING OBLIGATION (loadTable-metadata-cache PR, same branch).** The persisted `metadata_location` pointer carries an `as_of_t` forward-guard: `disk_catalog_cache::pointer_is_usable` refuses a pointer older than a requested `min_snapshot_ms` (hermetic-tested), but the `load_table_context` caller passes `None` (latest) today because Iceberg time-travel is unimplemented on the scan path (`GREP: r2rml-as-of-t`). **Whoever implements Iceberg snapshot time-travel MUST thread the requested snapshot ms into `get_metadata_location(&lt_key, Some(ms))`** — else bounded pointer staleness could silently downgrade a time-travel request. Guard + hermetic exist; only the caller argument is inert. See `21-loadtable-metadata-cache.md` §4.

---

**Historical (REFUTED) reading — preserved as the trail.**
...
**Remedy (as originally proposed — REFUTED; superseded by the CORRECTION above).** "Confirm the correlated inner-join context-derivation site; clone `r2rml_parent_memo`; add a q031-shaped hermetic test." Refuted because the correlated inner-join on q031's path (`with_active_graph`) already clones the memo, and the proposed hermetic test would pass with or without any base-ctor change. The one thing still worth doing — cloning the memo in `with_graph_ref` (`:1222`) — is the DEFERRED residual fix in the CORRECTION above, and it benefits only SERVICE / multi-source-default R2RML paths (no corpus query).

**Verdict / close.** F19 CLOSES: mechanism refuted (the memo already shares on every corpus path); one sound residual fix (`with_graph_ref` memo-clone) filed and deferred to the next PR touching that subsystem; **no current corpus query is affected either way.** The `q031` residual over-scan is a DIFFERENT lever — see the q031 residual-attribution (the ~241 post-F20 DIM_PRODUCT re-scans are rebuild-reset FILTERED/bounded MAIN scans excluded from `scan_cache` by `operator.rs:987`, which the parent-memo structurally does not cover; measured in `20-pr-q031-refprune.md` §residual).

**Fix owner.** engine — the residual `with_graph_ref` clone is standalone + deferred (rides the loadTable-cache PR); nothing to ship in the F20 PR.
...
**Expected wall (arithmetic).** Post-prune residual = one 7,670-file `FACT_INVENTORY_SNAPSHOT` decode (the un-prunable `FILTER(?oh<?rp)` blocks any LIMIT cut) + one DIM_PRODUCT (1-file) join + LIMIT-5000 materialize. Empirical reference from the SAME baseline: a single full 7,670-file fact scan is **~1 s** (q018 1.06 s / 7,670 files / 200 K rows; q044 0.96 s / 7,670 files / 250 K rows). So q031 is expected to land **low-single-digit s cache-thrashed WITHOUT PR-2a** — the ≤3 s part of this prediction was NOT met (see the residual measurement below); the qualitative read (fact-floor-dominated, not re-scan-dominated) WAS.

**RESIDUAL — MEASURED (2026-07-14 night; the q031 chain's SIXTH entry, closing the F18-era hyp-A vs hyp-B dispute).** F20 gate landed q031 at **17.2 s cache-thrashed / ~9 s warm-disk, NOT ≤3 s.** The 243 surviving `scan_table` were attributed from a per-scan trace (`FLUREE_R2RML_TRACE_SCANS`, reverted): **241 × DIM_PRODUCT budgeted MAIN re-scans** of `?p edw:name` (`filters=0 budget=Some(5000) cacheable=false BYPASS` — the LIMIT's `row_budget` alone disqualifies the `scan_cache`, `operator.rs:987`), + 1 fact star scan + 1 (deduped) DIM_PRODUCT parent lookup. **Pre-registered dispute:** hyp-A (lead) the 241 re-scans cost ~5–7 s; hyp-B (predecessor, fact-floor) they are near-free. **A/B differential** (`FLUREE_R2RML_EXP_SMALLDIM_CACHE` collapses 241→3, same warm-disk state, 3 interleaved reps): A(OFF) median **9.22 s** (scan 243) vs B(ON) median **9.72 s** (scan 3) → **DELTA −0.5 s. hyp-B WINS; hyp-A REFUTED** (the re-scans cost nothing — `r2rml.prefetch`×242 = 0.00 s, decode 0.6–0.8 s; the wall is `iceberg.fetch_bytes`, 100 % fact-file-dominated). **Verdict (lead-approved): a query-scoped small-dim main-scan cache is REJECTED — measured worthless.** q031's residual is the fact-scan + materialize + `loadTable` floor — the SAME floor as q016-post-PR-4d; the two queries share ONE residual story. q031's ≤3 s needs (i) the loadTable-METADATA cache (`load_table.n=2` ≈ ~2–3 s) + (ii) the shared PR-2a / materialization fact-scan floor — no q031-specific lever remains. Full attribution: `20-pr-q031-refprune.md` §8.

**Fix owner.** engine (rewrite: RefObjectMap-target resolution constraint) — north-star slate, **the q031 PR** (supersedes the F18 cold-floor framing). Ships on the DETERMINISTIC gate (`load_table` 7→2, `scan_table` 1448→243, rows-parity); wall deferred to the shared loadTable-cache + PR-2a floor per the measured residual above.
...
| **F19** | CLOSED — mechanism REFUTED (5th flip); residual sound fix deferred; no corpus query affected | q031 (the on/off≡1448 was an artifact of q031's shape, not memo-inertness) | REFUTED: the memo already SHARES on q031's path. Named base ctors (337/392/451/499/546/595) are ROOT constructors (no `&self` parent, run once at query top) — nothing to clone; q031's real per-batch derivation is `with_active_graph` (all virtual R2RML runs under `GraphOperator`, `graph.rs:288/498`), which ALREADY clones `r2rml_parent_memo` (`:1103`). Confirm-experiment: `with_active_graph` per-rebuild ⇒ parent 1× (shared); fresh `ExecutionContext::new` ⇒ parent 5× (control). The 1448/243 scans are the F20 fan-out (`?p edw:name` is a COLUMN, not a `RefObjectMap` — parent-memo never applied). Only true resetter = `with_graph_ref` (`:1222`, deliberate for `const_sid_cache`'s store switch; NOT on any corpus path); cloning the memo there IS sound (key is store-disambiguated `(graph_source_id, parent_tm, cols, as_of_t)`). Residual fix = `with_graph_ref` memo-clone + `with_graph_ref`-derivation hermetic test, DEFERRED to the next PR touching SERVICE/multi-source R2RML (loadTable-cache vehicle). | engine — residual fix standalone + DEFERRED; nothing in the F20 PR |
...
| **F22** | perf — un-fused predicate-less COUNT (exploration family; q038 sibling) | q056 | `COUNT(*){?s ?p ?o}` has no manifest COUNT shortcut (not a single resolved table), so it materializes *every triple* — 69,046 files / 16 tables / 492 MB, full object-column fetch (`fetch_bytes` 63× q057 over the same file set) — to count rows it never inspects. 168.3s hot (measured, not DNF). Fix class: **fused predicate-less `COUNT(*)` = Σ over TriplesMaps of (manifest `record_count` × POM count)** → sub-second, the same un-fused-COUNT family as q038 (PR-6). Full decomposition: `ROADMAP.md` exploration-family §. Out of the north-star count (exploration first-touch); filed for discoverability beside its q038 sibling, not for a burndown row. | engine (fused predicate-less COUNT — PR-6 family; exploration follow-on, no burndown row) |
