...
**Suggested next steps:** Phase 0 + **Phase 1 complete** (A #1346, B #1347, C `refactor/phase1c-merge-contract`). The overlay-merge chokepoint is now one `resolve_overlay` producer + one `FactKeyV3` identity + four documented `OverlayMergeStrategy` variants — and 1C's confirm-then-fix caught a real export data-correctness bug (stale value leaking into RDF exports). Next is **Phase 2** — `CoherentLedgerState` + `ArcSwap`, which retires the `dict_novelty` Arc-identity class that 0.4's tests pin and 0.6's invariants document; the riskiest phase, now on the stabilized ground 1A–1C built.
...
*Next:* open the 1C draft PR (base `refactor/phase1b-factkey`), then **Phase 2** — `CoherentLedgerState` + `ArcSwap`, which retires the `dict_novelty` Arc-identity bug class that 0.4's tests pin and 0.6's invariants document.
