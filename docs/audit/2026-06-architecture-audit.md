...
*Next:* **C5** (the last 1C piece) — consolidate the **two duplicate raw-post-pass resolvers** (`resolve_overlay_retractions` in the main scan at `binary_scan.rs` vs `surviving_untranslated` in `export.rs`, both row-world `Flake` latest-t-wins dedup) into one shared row-world resolver, and add an `OverlayMergeStrategy` enum naming the four strategies (RowMerge / OrderedMerge / CountDelta / RawPostPass) as an explicit documented set over the single producer.
