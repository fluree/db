...
| 1 | Single overlay-merge chokepoint (the highest correctness ROI; `query_overlay_matrix` shows ~9–128× base→overlay headroom) | **In progress** — sub-plan **A→B→C** (investigation map: 5 merge impls; tie-breaking already consistent; **two legitimate fact-identity domains** — V3 `(s_id,p_id,o_type,o_key,o_i)` for the translated lane, row-world `Flake` `(s,p,o,dt,m)` for the raw-post-pass lane; the audit's "silent-drop bug" was found to be **dead code**, not live). **A DONE** (`refactor/phase1-translation-contract`, PR #1346): deleted the dead `translate_overlay_flakes` lane + typed `Translation { Translated / Untranslated(reason) }` contract. **REMAINING:** B (promote `FactKeyV3`, document the two identity domains) → C (the 4 merge strategies behind one contract, one PR each behind the 0.7 differential harness + `query_overlay_matrix` baseline). | #1346 (A) |
...
**Suggested next steps:** Phase 0 complete; **Phase 1 in progress** (step A landed in #1346). Next is **1B** — promote `FactKeyV3` into a shared overlay module and document the two identity domains (translated V3 vs raw-post-pass `Flake`, incl. the dt/langString subtlety / why `Unsupported` exists) — then **1C**, the four merge strategies (RowMerge / OrderedMerge / CountDelta / RawPostPass) behind one contract, migrated one PR at a time behind the 0.7 differential harness + the `query_overlay_matrix` baseline (capture the Phase-1 `bench-baseline` before the first kernel-touching PR). Phase 2's `CoherentLedgerState` then retires the `dict_novelty` Arc-identity class that 0.4's tests pin and 0.6's invariants document.

---

### Session handoff — RESUME HERE (2026-06-17)

**Live draft-PR stack off `main`** (each branch based on the previous): #1316 `refactor/phase0-newtypes` → #1344 `refactor/phase0-nscode` (**Phase 0 complete**) → #1346 `refactor/phase1-translation-contract` (**Phase 1A; current stack head**). All PRs draft; do not merge. **Branch new work from `refactor/phase1-translation-contract`.** Each branch carries a `chore:` cleanup commit that untracked the `.claude/worktrees/*` agent gitlinks which were breaking CI's recursive submodule init (root cause `fe2208cba`; now gitignored — the only real submodule is `testsuite-sparql/rdf-tests`; **never `git add` `.claude/worktrees/`**, and stage code explicitly, never `-A` from the repo root).

**On deck — Phase 1B (one canonical `FactKey`).** There are currently **two** `FactKeyV3` structs: the `pub` one in `fluree-db-binary-index/src/read/types.rs` (with `fact_key()` on `DecodedRowV3`/`OverlayOp`; used by `resolve_overlay_ops` + the cursor merge) and a **private duplicate** in `read/replay.rs`. 1B = unify them into the single canonical translated-lane key, and **document (don't erase) the two legitimate identity domains**: V3 `(s_id,p_id,o_type,o_key,o_i)` for the translated lane vs row-world `Flake` `(s,p,o,dt,m)` for the export raw-post-pass lane — the latter is *required* to dedup untranslated flakes (it's why novelty-only lang tags return `Unsupported`, issue #1273; V3 folds datatype into o_type/o_key so `30^^int` vs `30.0^^float` collide there but are distinct Flakes). Builds on the `Translation { Translated / Untranslated(reason) }` enum that step A added in `binary_scan.rs`.

**Then Phase 1C (merge strategies behind one contract).** Migrate the four implementations one PR at a time, **kernels moved not changed**, each gated by the 0.7 differential harness (`fluree-db-api/tests/it_differential_fastpath.rs` — fast-path ≡ generic) + the `query_overlay_matrix` baseline (**capture the Phase-1 `bench-baseline` before the first kernel-touching PR**). The four: **RowMerge** = `binary_cursor::merge_overlay_into_batch`; **OrderedMerge** = `fast_post_order_limit::collect_post_desc_topk_overlay`; **CountDelta** = `fast_path_common::count_predicate_overlay_delta`; **RawPostPass** = export (`translate_overlay_flakes_with_untranslated` + `surviving_untranslated`). `range_with_overlay` is dispatch + a genesis special case, **not** a 5th impl.

**Carry-forward findings from the Phase-1 investigation:** tie-breaking is **already consistent** everywhere (latest-t-wins; assert-beats-retract on equal t) — write it once, don't re-derive. The audit's "residual silent-drop bug" was **dead code** (`translate_overlay_flakes`, deleted in 1A), so Phase 1 is structural *drift-prevention*, not bug remediation. Gate with the **`cnasty`** alias (= full-workspace `cargo nextest run --all-features`); skip the Docker/LocalStack tests locally (they hang on testcontainers).
