...
**Suggested next steps:** **Phase 0 is effectively complete** (0.0–0.7 done; the only remainder is 0.6's opportunistic `SAFETY:`-comment sweep over the ~20 unsafes + `join.rs` unwraps — low value, schedule with feature work). Begin **Phase 1** — the single overlay-merge chokepoint — the biggest correctness ROI, which the Phase 0.0 baseline + 0.7 differential harness were built to make safe. Phase 2's `CoherentLedgerState` then retires the `dict_novelty` Arc-identity class that 0.4's tests now pin and 0.6's invariants document.
