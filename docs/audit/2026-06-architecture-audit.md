...
| 0.1 | ID-space newtypes | **Partial** — `GraphId` done (`ffe90b60b`); **`NsCode(u16)` plain newtype DONE** (`refactor/phase0-nscode`, commit `97f064a0a`): `Sid.namespace_code` + vocab consts + `NamespaceCodes`/`NsLookup` API are `NsCode`; storage maps / `SubjectId` / wire-delta stay raw `u16` ("Option B"), converting at each single-code API boundary; all wire/serde/Display/key forms preserved via `.as_u16()`. **REMAINING:** snapshot-space vs store-space `NsCode` *tagging* at the `binary_scan` boundary — the half that maps to the historical namespace-after-index-attach bug — and `TxnGraphId` (txn-local graph-id space, see #1316 `build_reverse_graph_lookup`). `SubjectId` intentionally kept `u16` (packs ns_code into a u64 across the hot store path). | #1316 (GraphId) + `97f064a0a` (NsCode plain) |
...
**Suggested next steps**, in order: (a) finish **0.1** — the `NsCode` **snapshot/store space-tagging** at the `binary_scan` boundary (the half that maps to the historical namespace-after-index-attach bug; the plain newtype is now in place to build on), then `TxnGraphId`; (b) **0.4** concurrency regression tests (the safety net Phase 2 depends on); (c) **0.6** invariants/SAFETY docs (cheap, low-risk); then begin **Phase 1**, the overlay chokepoint, which is the biggest correctness win and is what the Phase 0.0 + 0.7 harnesses were built to make safe.
