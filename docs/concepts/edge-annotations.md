...
| **Turtle / N-Triples / TriG / N-Quads file ingest** | RDF 1.2 asserting forms: `:s :p :o ~ <reifier> {\| ... \|}`, `<< :s :p :o ~ :r >>` in subject or object position, and the canonical `:r rdf:reifies <<( :s :p :o )>>` (the only star spelling N-Triples and N-Quads have) — in the default graph and inside `GRAPH { }` blocks alike | Produces the same on-disk bundle as `@annotation` (bit-identical, tested): an annotation inside a `GRAPH { }` block, or on an N-Quads statement with a graph label, is written into that graph with the edge's graph identity, exactly as JSON-LD `@graph` + `@annotation` does. Divergence from RDF 1.2: the reified triple is **asserted** (Fluree annotations describe an edge that is in the graph). `<<( ... )>>` anywhere other than the object of `rdf:reifies`, nested triple terms, star constructs inside an annotation body, and annotations in the `#txn-meta` graph are rejected with a specific error. |
