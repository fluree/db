...
| **Turtle / N-Triples file ingest** | RDF 1.2 asserting forms: `:s :p :o ~ <reifier> {\| ... \|}`, `<< :s :p :o ~ :r >>` in subject or object position, and the canonical `:r rdf:reifies <<( :s :p :o )>>` (the only star spelling N-Triples has) | Produces the same on-disk bundle as `@annotation` (bit-identical, tested). Divergence from RDF 1.2: the reified triple is **asserted** (Fluree annotations describe an edge that is in the graph). `<<( ... )>>` anywhere other than the object of `rdf:reifies`, nested triple terms, and star constructs inside an annotation body are rejected with a specific "deferred" error. |
| **TriG / N-Quads file ingest** | Default-graph statements only | Default-graph statements take the Turtle rules above (an N-Quads file is regrouped into TriG first). Star constructs **inside a `GRAPH { }` block** (TriG-star, or an N-Quads statement with a graph label) are rejected with a clear deferred error — use the JSON-LD surface for named-graph edge annotations. |
...
- **Non-JSON-LD output.** v1 emits annotations in JSON-LD output. Turtle, TriG, N-Quads, and SPARQL CONSTRUCT need a separate surface-form decision (Turtle-star vs RDF 1.2 reifier vs other) and currently return `UnsupportedFeature` when a CONSTRUCT against those targets projects annotation metadata. (The ingest side has made its choice — all RDF 1.2 asserting spellings are read — so the natural output form is `r rdf:reifies <<( s p o )>>`.)
...
- **Star constructs inside named graphs on the text surfaces.** SPARQL UPDATE annotation tails under `GRAPH { }` / `WITH <g>`, and TriG-star / N-Quads statements with a graph label, are rejected; the JSON-LD `@annotation` surface is the named-graph write path.
