...
| **Turtle file ingest** (`insert`, `upsert`, bulk `import`, `fluree graph sync`, memory import) | RDF 1.2 annotation tail: `{\| ... \|}`, `~ reifier`, `<< s p o >>` | Same on-disk shape as `@annotation`. Every form asserts the base edge (Fluree reifies asserted edges, so `<< s p o >>` is asserting here). Rejected at parse time: `<<( ... )>>` triple terms, nested annotations, annotations on collections. Paths that convert to JSON-LD first (`upsert`, `graph sync`, memory import) also reject an annotation on an `rdf:type` edge. TriG: no annotations inside `GRAPH { }` blocks (default graph only, as for SPARQL UPDATE). See [Turtle ingest](../transactions/turtle.md#edge-annotations-rdf-12--turtle-star). |

Annotations are backed by a set of [reserved system predicates](../reference/vocabulary.md#edge-annotation-predicates-reserved) that application writes can't author by hand — mint annotations through `@annotation` / `@edge` (JSON-LD) or the RDF 1.2 annotation tail (`~`, `{| |}`) in SPARQL and Turtle. (Bulk import is an administrative bootstrap path that may ingest already-lowered annotation bundles without up-front validation; see the [storage-internals design doc](../design/edge-annotations.md) for how malformed bundles are handled.)
