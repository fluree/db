    ...
    C: ContentStore,
{
    ...
    // Reads storage directly: a cached root outlives its release, and the walk
    // ends where storage no longer holds one. See [`PrevIndexChainWalk`].
    let mut walk = PrevIndexChainWalk::new(store, head, None);
