    ...
    #?(:clj
       ...
       (async done
              (go
                (let [conn         (<! (test-utils/create-conn))
                      ledger-alias "testledger"
                      db0          (<p! (fluree/create conn ledger-alias))
                      db           (<p! (fluree/update db0
                                                       {"@context" ["https://ns.flur.ee"
                                                                    test-utils/default-context]
                                                        "insert"
                                                        [{:id           :f/me
     ...
     (testing "loading predefined properties"
       (let [conn           (test-utils/create-conn)
         ...
         (is (= [{"id"             shape-id
                  ...
                  "sh:targetClass" {"id" "schema:Person"},
                  "sh:property"    {"sh:path" {"id" "schema:familyName"}, "sh:datatype" {"id" "xsd:string"}}}]
                @(fluree/query loaded1 property-query)))
         (testing "load ref retracts"
           (let [_db2    @(fluree/update! conn
             ...
             (is (= [{"id"             shape-id
                    ...
                    @(fluree/query loaded2 property-query)))))))
    ...
    (testing "basic query works"
      #?(:clj
         (let [conn      (test-utils/create-conn)
               ...
               results   @(fluree/query @(fluree/db conn ledger-id) query)]
           (is (= expected results)))
         :cljs
         (async done
                (go
                  (let [conn      (<! (test-utils/create-conn))
                        ledger-id (<! (test-utils/load-people conn))
                        results   (<p! (fluree/query (<p! (fluree/db conn ledger-id)) query))]
                    (is (= expected results))
                    (done))))))))
...
(deftest ^:integration fuel-test
  #?(:clj
     (testing "fuel tracking"
       (let [conn   (test-utils/create-conn)
             db0    (async/<!! (async-db/deref-async @(fluree/create conn "test/fuel-tracking")))]
     ...
     (async done
            (go
              (testing "fuel tracking"
                (let [conn (<! (test-utils/create-conn))
                      db0  (<p! (fluree/create conn "test/fuel-tracking"))]
                  (testing "transactions"
                    (testing "with the `:meta` option"
...
#?(:clj
   (deftest transaction-test
     (let [conn      @(fluree/connect-memory)
       ...
       (is (= 54
              (-> @(fluree/history conn ledger-id {:context        test-utils/default-str-context
                                                   :commit-details true
                                                   :t              {:from :latest}})
                  (first)
                  (get "f:commit")
   ...
   (deftest drop-test
     (with-temp-dir [storage-path {}]
       (let [primary-path   (str storage-path "/primary")
             ...
             _        @(fluree/create conn ledger-alias {:reindex-min-bytes 100 :max-old-indexes 3})
         ...
         (Thread/sleep 1000)
         (testing "before drop"
           ;; Storage uses base ledger name, not branch-specific paths
           (is (contains? (set (async/<!! (fs/list-files primary-path)))
                          ledger-alias)
               "Primary path should contain storage directory")
           (is (contains? (set (async/<!! (fs/list-files (str secondary-path "/ns@v1"))))
                          "destined-for-drop")
               "Secondary ns@v1 should contain destined-for-drop directory")
           ;; Storage uses base ledger name for commits/indexes (branches share storage)
           (is (= ["commit" "index" "txn"]
           ...
           ;; Each transaction creates one data file and one commit file
           ;; But the initial creation doesn't create a data file (just the commit)
           ;; So we have: 1 commit (initial) + 3 * 2 (data + commit for each tx) = 7 files
           ;; But actually, it seems we're getting 5 files, which might be:
           ;; 1 initial commit + 2 files per tx (data and commit) = 1 + 3*2 = 7 
           ;; Let's check what we're actually getting
           (let [commit-files (async/<!! (fs/list-files (str primary-path "/" ledger-alias "/commit")))]
         ...
         (testing "after drop"
           ...
           ;; The destined-for-drop directory in secondary ns@v1 remains but should be empty
           (is (contains? (set (async/<!! (fs/list-files (str secondary-path "/ns@v1"))))
                          "destined-for-drop")
               "Secondary ns@v1 should contain destined-for-drop directory")
           (is (= []
