  ...
  (let [conn   @(fluree/connect-memory)
    ...
    (testing "one+"
      (testing "no variables"
        (let [db1 @(fluree/update db0 {"@context" {"ex" "http://example.org/"}
                                       "insert"
                                       [{"@id" "ex:a"
                                         "ex:y" [{"@id" "ex:b"
                                                  "ex:y" {"@id" "ex:c"
          ...
          (testing "non-transitive"
            (is (= []
                   @(fluree/query db1 {"@context" {"ex" "http://example.org/"}
                                       "where" [{"@id" "ex:a" "ex:y" {"@id" "ex:f"}}]
                                       "select" {"ex:a" ["*"]}}))))
          (testing "transitive"
            (let [result @(fluree/query db1 {"@context" {"ex" "http://example.org/"}
                                             "where" [{"@id" "ex:a" "<ex:y+>" {"@id" "ex:f"}}]
                                             "select" {"ex:a" ["*"]}})]
            ...
            (testing "without cycle"
              (is (= ["ex:b" "ex:c" "ex:d" "ex:e"]
                     @(fluree/query db1 {"@context" {"ex" "http://example.org/"}
                                         ...
                                         "select" "?who"}))))
            (testing "with cycle"
              (let [db2 @(fluree/update db1 {"@context" {"ex" "http://example.org/"}
                                             "insert" {"@id" "ex:e" "ex:knows" {"@id" "ex:a"}}})]
          ...
          (testing "transitive"
            (testing "without cycle"
              (is (= #{["ex:1" "ex:2"]
                       ["ex:2" "ex:3"]
                       ["ex:1" "ex:3"]}
                     (set @(fluree/query db1 {"@context" {"ex" "http://example.org/"}
                                              "where" [{"@id" "?x" "<ex:knows+>" "?y"}]
                                              "select" ["?x" "?y"]})))))
            (testing "with cycle"
              (let [db2 @(fluree/update db1 {"@context" {"ex" "http://example.org/"}
                                             "insert" {"@id" "ex:3" "ex:knows" {"@id" "ex:1"}}})]
                (is (= #{["ex:3" "ex:2"]
                         ["ex:1" "ex:2"]
                         ["ex:2" "ex:3"]
                         ["ex:1" "ex:3"]
                         ...
                         ["ex:2" "ex:1"]
                         ["ex:1" "ex:1"]}
                       (set @(fluree/query db2 {"@context" {"ex" "http://example.org/"}
                                                "where" [{"@id" "?x" "<ex:knows+>" "?y"}]
                                                "select" ["?x" "?y"]}))))))))))
    (testing "zero+"
      (testing "no variables"
          ...
          (testing "transitive"
            (testing "without cycle"
              (is (= [["ex:1" "ex:1"]
                      ...
                      ["ex:2" "ex:3"]
                      ["ex:3" "ex:3"]]
                     (sort @(fluree/query db1 {"@context" {"ex" "http://example.org/"}
                                               "where" [{"@id" "?x" "<ex:knows*>" "?y"}]
                                               "select" ["?x" "?y"]})))))
            (testing "disjoint subgraphs"
              (let [db2 @(fluree/update db1
                                        {"@context" {"ex" "http://example.org/"}
                                         "insert"
                                         [{"@id" "ex:4"
                                           "ex:knows" {"@id" "ex:5"
            ...
            (testing "with cycle"
              (let [db2 @(fluree/update db1 {"@context" {"ex" "http://example.org/"}
                ...
                (is (= [["ex:1" "ex:1"]
                        ...
                        ["ex:3" "ex:2"]
                        ["ex:3" "ex:3"]]
                       (sort @(fluree/query db2 {"@context" {"ex" "http://example.org/"}
                                                 "where" [{"@id" "?x" "<ex:knows*>" "?y"}]
                                                 "select" ["?x" "?y"]}))))))))))))
