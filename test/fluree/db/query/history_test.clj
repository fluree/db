  ...
  (let [conn (test-utils/create-conn)
    ...
    (testing "subject history"
      (is (= [{:t 1 :assert {:id :ex/dan :ex/x "foo-1" :ex/y "bar-1"}}
             ...
             (sort-by :t @(fluree/history ledger {:history :ex/dan})))))
    ...
    (testing "two-tuple flake history"
      (is (= [{:t 1 :assert {:ex/x "foo-1"}}
             ...
             (sort-by :t @(fluree/history ledger {:history [:ex/dan :ex/x]}))))
      ...
      (is (= [{:t 1 :assert {:ex/x "foo-1"}}
              ...
              {:t 5 :assert {:ex/x "foo-cat"} :retract {:ex/x "foo-3"}}]
             (sort-by :t @(fluree/history ledger {:history [nil :ex/x]})))))
    (testing "three-tuple flake history"
      (is (= [{:t 4 :assert {:ex/x "foo-cat"}}
              {:t 5 :assert {:ex/x "foo-cat"}}]
             (sort-by :t @(fluree/history ledger {:history [nil :ex/x "foo-cat"]}))))
      (is (= [{:t 2 :assert {:ex/x "foo-2"}}
              {:t 3 :retract {:ex/x "foo-2"}}]
             (sort-by :t @(fluree/history ledger {:history [nil :ex/x "foo-2"]}))))
    ...
    (testing "from-t"
      (is (= [{:t 3 :assert {:ex/x "foo-3"}}
              {:t 5 :assert {:ex/x "foo-cat"} :retract {:ex/x "foo-3"}}]
             (sort-by :t @(fluree/history ledger {:history [:ex/dan :ex/x] :t {:from 3}})))))
    (testing "to-t"
      (is (= [{:t 1 :assert {:ex/x "foo-1"}}
              {:t 2 :assert {:ex/x "foo-2"} :retract {:ex/x "foo-1"}}
              {:t 3 :assert {:ex/x "foo-3"} :retract {:ex/x "foo-2"}}]
             (sort-by :t @(fluree/history ledger {:history [:ex/dan :ex/x] :t {:to 3}})))))
    (testing "t-range"
      (is (= [{:t 1 :assert {:ex/x "foo-1"}}
              {:t 2 :assert {:ex/x "foo-2"} :retract {:ex/x "foo-1"}}
              {:t 3 :assert {:ex/x "foo-3"} :retract {:ex/x "foo-2"}}]
             (sort-by :t @(fluree/history ledger {:history [:ex/dan :ex/x] :t {:from 1 :to 3}})))))
