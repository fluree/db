...
#_(deftest ^:integration class-policy-query-enforcement
  (testing "Restrict an entire class for viewing via relationship "
    (let [conn      (test-utils/create-conn)
          ledger    @(fluree/create conn "policy/sparql-policy-enforcement-2")
          root-did  (:id (did/private->did-map "8ce4eca704d653dec594703c81a84c403c39f262e54ed014ed857438933a2e1c"))
          alice-did (:id (did/private->did-map "c0459840c334ca9f20c257bed971da88bd9b1b5d4fca69d4e3f4b8504f981c07"))
          db        @(fluree/stage
                      (fluree/db ledger)
      ...
      (testing " and values binding has user without policy relationship"
        (is (= []
               @(fluree/query-connection
                 conn
                 "SELECT ?s ?o ?p FROM <policy/sparql-policy-enforcement> WHERE { ?s ?o ?p }"
                 {:format :sparql :role "ex:rootRole" :did root-did})))))))
