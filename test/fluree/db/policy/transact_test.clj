...
(deftest ^:integration policy-enforcement
  (testing "Testing basic policy enforcement."
    (let [conn      (test-utils/create-conn)
          ledger    @(fluree/create conn "policy/tx-a" {"defaults"
                                                        {"@context"
          ...
          db        @(fluree/stage
                      ...
                      [{"id"               "ex:alice",
                       ...
                       {"id"      alice-did
                        "ex:user" {"id" "ex:alice"}
                        "f:role"  [{"id" "ex:userRole"} {"id" "ex:otherRole"}]}])
          ...
          db+policy @(fluree/stage
                      ...
                      [{"id"           "ex:rootPolicy",
                        ...
                        "f:allow"      [{"id"           "ex:rootAccessAllow"
                                         "f:targetRole" {"id" "ex:rootRole"} ;; our name for global / root role
                       ...
                       {"id"            "ex:UserPolicy",
                        ...
                        "f:allow"       [{"id"           "ex:globalViewAllow"
                                          "f:targetRole" {"id" "ex:userRole"} ;; our assigned name for standard user's role (given to Alice above)
                                          "f:action"     [{"id" "f:view"}]}]
                       ...
                       {"id"            "ex:ProductPolicy",
                        "type"          ["f:Policy"],
                        "f:targetClass" {"id" "ex:Product"}
                        "f:property"    [{"f:path"  {"id" "rdf:type"}
                                          "f:allow" [{"f:targetRole" {"id" "ex:userRole"}
                                                      "f:action"     [{"id" "f:view"}]}]}
                                         {"f:path"  {"id" "schema:name"}
                                          "f:allow" [{"f:targetRole" {"id" "ex:userRole"}
                                                      "f:action"     [{"id" "f:view"} {"id" "f:modify"}]}]}]}])]

      (testing "Policy allowed modification"
        (testing "using role + id"
          (let [update-email @(fluree/stage db+policy
                                            {"id"           "ex:alice"
