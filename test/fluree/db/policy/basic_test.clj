...
(deftest ^:integration query-policy-enforcement
  (testing "Testing basic policy enforcement on queries."
    (let [conn      (test-utils/create-conn)
          ledger    @(fluree/create conn "policy/a" {"defaults"
                                                     {"@context"
      ...
      (testing "multi-query"
        ...
        (let [illegal-opts-multi-result @(fluree/multi-query
                                          db+policy
                                          {"john"  {"select" '[?ssn]
                                                    "where"  '[[?p "schema:name" "John"]
                                                               {"optional" [?p "schema:ssn" ?ssn]}]
      ...
      (testing "history query"
        (let [_ @(fluree/commit! ledger db+policy)]
          (is (= []
                 @(fluree/history ledger {"history"        ["ex:john" "schema:ssn"]
                                          "t"              {"from" 1}
                                          "commit-details" true
                                          "opts"           {"did"  alice-did
                                                            "role" "ex:userRole"}}))
