      ...
      (testing "multi-cardinality equals"
        (let [db            @(fluree/stage
                              ...
                              {"id"             "ex:EqualNamesShape"
                               "type"           ["sh:NodeShape"],
                               "sh:targetClass" {"id" "ex:User"}
                               "sh:property"    [{"sh:path"   {"id" "ex:favNums"}
                                                  "sh:equals" {"id" "ex:luckyNums"}}]})
              db-ok         @(fluree/stage
                              db
                              {"id"           "ex:alice",
                               "type"         ["ex:User"],
      ...
      (testing "lessThan"
        (let [db       @(fluree/stage
                         ...
                         {"id"             "ex:LessThanShape"
                          "type"           ["sh:NodeShape"],
                          "sh:targetClass" {"id" "ex:User"}
                          "sh:property"    [{"sh:path"     {"id" "ex:p1"}
                                             "sh:lessThan" {"id" "ex:p2"}}]})
              db-ok1   @(fluree/stage
                         db
                         {"id"          "ex:alice",
                          "type"        ["ex:User"],
      ...
      (testing "exclusive constraints"
        (let [db          @(fluree/stage
                            ...
                            {"id"             "ex:ExclusiveNumRangeShape"
                             "type"           ["sh:NodeShape"],
                             "sh:targetClass" {"id" "ex:User"}
                             "sh:property"    [{"sh:path"         {"id" "schema:age"}
                                                "sh:minExclusive" 1
                                                "sh:maxExclusive" 100}]})
              db-ok       @(fluree/stage
                            db
