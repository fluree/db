...
(deftest ^:integration deleting-data
  (testing "Deletions of entire subjects."
    (let [conn   (test-utils/create-conn)
          ...
          db-subj-delete @(fluree/update db
                                         {"@context" [test-utils/default-context
                                                      {:ex "http://example.org/ns/"}]
          ...
          db-subj-pred-del @(fluree/update db
                                           {"@context" [test-utils/default-context
                                                        {:ex "http://example.org/ns/"}]
          ...
          db-age-delete @(fluree/update db
                                        {"@context" [test-utils/default-context
                                                     {:ex "http://example.org/ns/"}]
                                         "delete"   {:id "?s", "?p" "?o"}
                                         "where"    {:id         "?s"
          ...
          db-update-bob @(fluree/update db
                                        {"@context" [test-utils/default-context
                                                     {:ex "http://example.org/ns/"}]
          ...
          db-update-bob2 @(fluree/update db
                                         {"@context" [test-utils/default-context
                                                      {:ex "http://example.org/ns/"}]
          ...
          db-update-jane @(fluree/update db
                                         {"@context" [test-utils/default-context
                                                      {:ex "http://example.org/ns/"}]
  ...
  (let [conn   @(fluree/connect-memory)
    ...
    (testing "datetime functions"
      (with-redefs [fluree.db.query.exec.eval/now const-now]
        (let [db2 @(fluree/update db1 {"@context" [test-utils/default-str-context
                                                   {"ex" "http://example.com/"}]
                                       "insert"
                                       [{"id"         "ex:create-predicates"
                                        ...
                                        {"id"                "ex:datetime-fns"
                                         ...
                                         "ex:utcdatetime"    {"@value" "2023-06-13T14:17:22.435Z"
                                                              "@type"  const/iri-xsd-dateTime}}]})
              db3 @(fluree/update db2 {"@context" [test-utils/default-str-context
                                                   {"ex" "http://example.com/"}]
    ...
    (testing "error handling"
      (let [db2       @(fluree/update db1 {"@context" [test-utils/default-str-context
                                           ...
                                           "insert"   [{"id"       "ex:create-predicates"
                                                       ...
                                                       {"id"      "ex:error"
                                                        "ex:text" "Abcdefg"}]})
            parse-err @(fluree/update db2 {"@context" [test-utils/default-str-context
                                                       {"ex" "http://example.com/"}]
                                           "where"    [{"id" "?s", "ex:text" "?text"}
                                                       ["bind" "?err" "(foo ?text)"]]
            ...
            _run-err @(fluree/update db2 {"@context" [test-utils/default-str-context
                                                      {"ex" "http://example.com/"}]
                                          "where"    [{"id" "?s", "ex:text" "?text"}
                                                      ["bind" "?err" "(abs ?text)"]]
