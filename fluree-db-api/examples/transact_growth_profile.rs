...
async fn one_commit(fluree: &Fluree, ledger: LedgerState, json: &JsonValue) -> (LedgerState, usize) {
...
fn summarize(rows: &[Row]) {
    ...
    eprintln!("final novelty    : {} flakes / {:.1} MiB", last.novelty_flakes, last.novelty_bytes as f64 / (1024.0 * 1024.0));
    eprintln!("latency  mean    : {} us", total_us / n);
    eprintln!("latency  p50/p95/p99 : {} / {} / {} us", pct(0.50), pct(0.95), pct(0.99));
    eprintln!("first-commit us  : {}", first.elapsed_us);
    eprintln!("last-commit  us  : {}  ({:.1}x first)", last.elapsed_us, last.elapsed_us as f64 / first.elapsed_us.max(1) as f64);
    eprintln!("throughput       : {:.0} commits/s, {:.0} flakes/s (timed region)", n as f64 * 1_000_000.0 / total_us as f64, total_flakes as f64 * 1_000_000.0 / total_us as f64);
    if last.rss_kb > 0 {
...
async fn grow(fluree: &Fluree, ledger_id: &str, commits: usize, npc: usize, sample_every: usize, reindex_every: usize) -> Vec<Row> {
    ...
    for i in 0..commits {
        ...
        if i % report_every == 0 {
            eprintln!("[grow] commit {i}/{commits}  novelty={} flakes / {:.0} MiB  last={elapsed_us}us", ledger.novelty().len(), ledger.novelty_size() as f64 / (1024.0 * 1024.0));
        }

        if reindex_every > 0 && i > 0 && i % reindex_every == 0 {
            eprintln!("[grow] reindex at commit {i} (novelty {:.0} MiB) ...", ledger.novelty_size() as f64 / (1024.0 * 1024.0));
            drop(ledger);
            fluree.reindex(ledger_id, ReindexOptions::default()).await.expect("reindex");
            ledger = fluree.ledger(ledger_id).await.expect("reload after reindex");
        }
    }
    rows
}
...
async fn prepare(fluree: &Fluree, ledger_id: &str, commits: usize, npc: usize) {
    ...
    let mut ledger = fluree.create_ledger(ledger_id).await.expect("create_ledger");
    let report_every = (commits / 20).max(1);
    eprintln!("[prepare] building {commits} commits of novelty ({npc} nodes each) at GROW_DB_DIR ...");
    for i in 0..commits {
        let json = txn_data_to_jsonld(&generate_txn_data(i, npc));
        let (next, _) = one_commit(fluree, ledger, &json).await;
        ledger = next;
        if i % report_every == 0 {
            eprintln!("[prepare] commit {i}/{commits}  novelty={} flakes / {:.0} MiB", ledger.novelty().len(), ledger.novelty_size() as f64 / (1024.0 * 1024.0));
        }
    }
    eprintln!("[prepare] DONE: t={} novelty={} flakes / {:.1} MiB. Ready for GROW_MODE=steady.", ledger.t(), ledger.novelty().len(), ledger.novelty_size() as f64 / (1024.0 * 1024.0));
}
...
async fn steady(fluree: &Fluree, ledger_id: &str, warmup: usize, steady_commits: usize, sample_every: usize) -> Vec<Row> {
    let mut ledger = fluree.ledger(ledger_id).await.expect("reload prepared ledger (run GROW_MODE=prepare first)");
    let base_t = ledger.t();
    eprintln!("[steady] reloaded at t={base_t}, novelty={} flakes / {:.1} MiB", ledger.novelty().len(), ledger.novelty_size() as f64 / (1024.0 * 1024.0));
    ...
    match mode.as_str() {
        "all" => {
            ...
            let fluree = FlureeBuilder::file(dir.path().to_string_lossy().to_string())
                ...
                .build()
                .expect("build");
            let rows = grow(&fluree, &ledger_id, commits, npc, sample_every, reindex_every).await;
            write_csv(&csv, &rows);
            summarize(&rows);
        }
        "grow" => {
            ...
            let fluree = FlureeBuilder::file(dir).without_indexing().build().expect("build");
            let rows = grow(&fluree, &ledger_id, commits, npc, sample_every, reindex_every).await;
            write_csv(&csv, &rows);
            summarize(&rows);
        }
