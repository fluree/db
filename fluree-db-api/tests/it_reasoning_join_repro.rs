...
async fn run(fluree: &support::MemoryFluree, ledger: &support::MemoryLedger, q: &serde_json::Value) -> Vec<serde_json::Value> {
...
async fn case01_single_derived_type() {
    ...
    assert!(r.contains(&json!("ex:g")), "single derived-type should include ex:g, got {r:?}");
}
...
async fn case04_derived_type_plus_literal_prop() {
    ...
    let r = run(&fluree, &ledger, &q).await;
    assert_eq!(r, vec![json!("GraduateStudent101")], "derived type + literal prop");
}
...
async fn case05_base_type_plus_ref_prop() {
    ...
    assert_eq!(r, vec![json!("ex:c0"), json!("ex:c65")], "base type + ref prop");
}
...
async fn case06_derived_type_plus_ref_prop() {
    ...
    let mut r = run(&fluree, &ledger, &q).await;
    r.sort_by_key(|v| v.to_string());
    assert_eq!(r, vec![json!("ex:c0"), json!("ex:c65")], "BUG: derived type + ref prop");
}
...
async fn case08_derived_type_plus_advisor() {
    ...
    let r = run(&fluree, &ledger, &q).await;
    assert_eq!(r, vec![json!("ex:p1")], "BUG: derived type + advisor ref prop");
}
...
async fn case12_var_subject_bound_ref_object() {
    ...
    let r = run(&fluree, &ledger, &q).await;
    assert!(r.contains(&json!("ex:g")), "BUG: var subject + bound ref object should include ex:g, got {r:?}");
}
...
async fn iso_pure_derived_type_two_valued_ref() {
    ...
    let mut r = run(&fluree, &ledger, &q).await;
    r.sort_by_key(|v| v.to_string());
    assert_eq!(r, vec![json!("ex:c0"), json!("ex:c65")], "pure derived type + 2-valued ref");
}
...
async fn iso_pure_derived_type_literal() {
    ...
    let r = run(&fluree, &ledger, &q).await;
    assert_eq!(r, vec![json!("GraduateStudent101")], "pure derived type + literal");
}
...
async fn manual_union_two_valued_ref_no_reasoning() {
    ...
    assert_eq!(
        r,
        vec![json!("ex:c0"), json!("ex:c0"), json!("ex:c65"), json!("ex:c65")],
...
async fn cross_product_second_multivalued_no_union() {
    ...
    assert_eq!(r.len(), 2, "cross product name×takesCourse, NO union: got {r:?}");
}
...
async fn unit_pattern_times_multivalued_no_union() {
    ...
    let mut r = run(&fluree, &ledger, &q).await;
    r.sort_by_key(|v| v.to_string());
    assert_eq!(r, vec![json!("ex:c0"), json!("ex:c65")], "unit pattern × multivalued, NO union: {r:?}");
}
...
async fn base_and_derived_same_type_not_doubled() {
    ...
    let r = run(&fluree, &ledger, &q).await;
    assert_eq!(r, vec![json!("ex:e")], "base+derived same type must not double: {r:?}");
}
...
async fn manual_union_one_valued_ref_no_reasoning() {
    ...
    let r = run(&fluree, &ledger, &q).await;
    assert_eq!(r, vec![json!("ex:p1"), json!("ex:p1")], "manual union + 1-valued ref, NO reasoning");
}
