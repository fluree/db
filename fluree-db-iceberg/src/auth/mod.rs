...
impl AuthConfig {
    ...
    pub async fn hydrate(&self, resolver: Option<&Arc<dyn SecretResolver>>) -> Result<AuthConfig> {
        match self {
            ...
            // Carries no `ConfigValue`, so there is no secret reference to
            // resolve — tokens are minted at runtime from the metadata server.
            // Spelled out rather than folded into a wildcard so this match keeps
            // failing compilation when a new variant appears.
            AuthConfig::GoogleMetadata {
                scopes,
                metadata_url,
