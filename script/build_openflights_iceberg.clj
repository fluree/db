...
(defn record-writer
  "Writes rows to an Iceberg table using GenericAppenderFactory (Iceberg 1.10+ API)."
  ...
  (let [table         (.load tables table-path)
        ...
        ;; Create output file and data writer (use newDataWriter for toDataFile method)
...
(defn -main [& _args]
  ...
  (let [conf   (Configuration.)
        ;; HadoopTables uses full paths (no warehouse root concept).
        ;; For catalog-style API with warehouse root, use HadoopCatalog instead.
