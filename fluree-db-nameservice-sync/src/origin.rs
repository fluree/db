...
// Integrity verification lives in the runtime-agnostic `integrity` module
// (it is shared with the wasm32 build); re-exported here so existing
// `origin::verify_object_integrity` paths keep working.
pub use crate::integrity::verify_object_integrity;
