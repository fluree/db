...
impl BatchBridge {
    ...
    pub fn new(arrow_schema: SchemaRef, field_ids: &[i32]) -> Result<Self> {
        ...
        {
            return Err("field IDs must be unique and cover the logical projection".into());
        }
        ...
        let fields = arrow_schema
            ...
            .map(|(f, id)| {
                if !names.insert(f.name()) {
                    return Err(format!("duplicate logical field name: {}", f.name()).into());
                }
                Ok(FieldInfo {
                    name: f.name().clone(),
    ...
    pub fn convert(&self, batch: &RecordBatch) -> Result<ColumnBatch> {
        ...
        if batch.num_columns() != self.schema.num_fields() {
            return Err("logical batch column count changed".into());
        }
            ...
            .zip(batch.columns())
        {
            ...
            {
                return Err(format!("logical batch schema changed for {}", expected.name()).into());
            }
        }
        let columns = batch
            .columns()
...
fn field_type(data_type: &DataType) -> Result<FieldType> {
    Ok(match data_type {
        ...
        other => return Err(format!("unsupported Delta batch type: {other:?}").into()),
    })
}
...
mod tests {
    ...
    fn decimal_precision_and_timestamp_frames_are_preserved() {
        let decimal = Decimal128Array::from(vec![Some(-123456789012345678901234567890_i128), None])
        ...
        assert_eq!(
            values,
            vec![Some(-123456789012345678901234567890_i128), None]
        );
        let times = TimestampMicrosecondArray::from(vec![Some(-1234567), None]);
        ...
        assert_eq!(naive.get_timestamp(0), Some(-1234567));
        assert_eq!(utc.get_timestamp(0), Some(-1234567));
