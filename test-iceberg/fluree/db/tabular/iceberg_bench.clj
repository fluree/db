...
(defn benchmark-full-scan
  ...
  (let [table (load-table)
    ...
    ;; Warm up
    (println "Warming up...")
    (force-and-count (core/scan-with-arrow table opts))
    (force-and-count (core/scan-with-generics table opts))

    ;; Arrow
    (print "Arrow vectorized:   ")
    (flush)
    (let [[cnt ms] (time-ms #(force-and-count (core/scan-with-arrow table opts)))]
...
(defn benchmark-repeated
  ...
  (let [table (load-table)
    ...
    (let [times (for [_ (range iterations)]
                  (second (time-ms #(force-and-count (core/scan-with-arrow table opts)))))
